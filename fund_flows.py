...
def fetch_new_bearer_token(cj: http.cookiejar = None):
    ...
    cookies = {}
    try:
        cookies = {cookie.name: cookie.value for cookie in cj if "etf.com" in cookie.domain}
...
def fetch_fund_flow_data(ticker, bearerToken: str, date_from: date, date_to: date):
    if (not bearerToken):
        return None
    ...
    if res.status_code == 200:
        json = res.json()
        wb_name = f"data/{ticker}_fund_flow_data.xlsx"
